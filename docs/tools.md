...
(TUI) или headless-прогона (`arch-ml run`). Пользователь их не вызывает напрямую —
...
| `screenshot` / `read_image` / `screen_size` / `window_list` | Наблюдение за экраном: кадр, изображение, размер экрана, окна (vision `deepseek-flash`) |
...
| `worktree_new` | Изолированный git worktree (ветка `arch/<name>`, каталог вне репозитория) для рискованных/параллельных правок; агент работает в нём через `workdir` остальных инструментов. Review/accept/drop — человеком (`arch-ml worktree …`) | `name`* — kebab-case; `repo` (пусто — текущий каталог); `base` (пусто — HEAD) |
...
Отчёты дублируются файлами: субагенты — `~/.arch-ml/reports/subagents/<id>.md`,
ralph-циклы — `~/.arch-ml/reports/ralph/<id>/` (`round-NN.md`,
...
CLI-эквиваленты для CI/скриптов: `arch-ml archify validate|deliver|compare|guide|doctor`;
...
| `web_search` | DuckDuckGo; с `arch-ml=true` — только кураторские сайты архитектора | `query`*; `arch-ml` (bool) |
...
Мультимодальность `deepseek-flash`: изображение возвращается в результат
инструмента, агентный цикл прикрепляет его к user-сообщению, модель его видит.
Изображения допустимы только в `user`-сообщениях (провайдер отклоняет их в
`system`/`assistant`); форматы — JPEG, PNG, GIF, WebP; в запрос уходят только
последние `[agent].images_keep_last` кадров (старые снимаются с пометкой —
иначе тело запроса растёт линейно от числа скриншотов).

| Инструмент | Назначение | Параметры |
|---|---|---|
| `screenshot` | Снять экран и передать кадр модели. Результат сообщает размер кадра — он нужен для перевода координат `computer_*`. После каждого действия снимать заново: результат проверяется, а не предполагается | `path` (куда сохранить; по умолчанию `state/screenshots/shot-<ts>.png`) |
| `read_image` | Прочитать файл изображения (png/jpg/webp/gif) и передать модели | `path`* |
| `screen_size` | Размер корневого окна X11 в пикселях | — |
| `window_list` | Видимые окна X11: id и заголовок — чтобы понять, куда адресованы действия | — |

Наблюдение включено всегда и гейтом `[computer]` не управляется: видеть экран
безопасно. Требуется X11-сессия; снимок делается первой доступной утилитой
(`scrot`, `import`, `gnome-screenshot`, `ffmpeg` x11grab).

## Управление компьютером и браузером (воздействие)

**Выключено по умолчанию** (`[computer].enabled = false`) и, независимо от
конфига, отнесено к классу `Destructive`: на R2 такие вызовы отклоняются, на
R4 требуют подтверждения человека, на R5 идут автоматически. Клик и ввод
необратимы и видны за пределами харнесса — гейт не декорация.

**Координатный контракт.** `x`, `y` — нормализованные 0–1000 по каждой оси,
начало — левый верхний угол последнего кадра, **не пиксели**. Провайдер ужимает
изображение перед инференсом (потолок 1024 токена на кадр, ~1300×1300), поэтому
пиксельная шкала модели к экрану не привязана; нормализованная — привязана.
Значение больше 1000 отвергается с подсказкой (мягкий зажим молча увёл бы клик
в край экрана). Требуется X11 и `xdotool` (`sudo apt install xdotool`) —
Wayland-бэкенды в этой ревизии не поддержаны сознательно.

| Инструмент | Назначение | Параметры |
|---|---|---|
| `computer_move` | Навести курсор (подсказки, подменю) | `x`*, `y`* |
| `computer_click` | Клик; наведение и клик одним процессом | `x`*, `y`*; `button` (`left`/`middle`/`right`), `clicks` (1–3, потолок `max_clicks`) |
| `computer_scroll` | Прокрутка колесом | `x`, `y` (обе или ни одной — иначе под курсором); `direction`* (`up`/`down`/`left`/`right`), `amount` |
| `computer_drag` | Перетаскивание (слайдеры, холсты) | `from_x`*, `from_y`*, `to_x`*, `to_y`*; `button` |
| `computer_key` | Клавиша или комбинация X11 | `keys`* (напр. `ctrl+l`, `Return`) |
| `computer_type` | Ввод текста в активное поле | `text`*; `method` — `type` (жмёт клавиши, надёжен только для ASCII) или `clipboard` (буфер + `ctrl+v`, для кириллицы) |
| `window_focus` | Поднять и активировать окно по подстроке заголовка | `title`* |

**Браузерный контур** (`[computer.browser].enabled`, отдельный гейт) даёт
структурный доступ к странице там, где зрение неточно. Харнесс поднимает
браузер с **выделенным** `--user-data-dir` внутри `state_dir` — ваш профиль с
логинами не читается и не изменяется; отладочный порт слушает только loopback.
CDP-хелпер (`assets/computer/cdp.mjs`, Node ≥ 18) встроен в бинарь и
разворачивается сам — без `arch-ml init`.

| Инструмент | Класс | Назначение | Параметры |
|---|---|---|---|
| `browser_open` | Mutating | Поднять браузер (при необходимости) и открыть URL | `url` (http/https) |
| `browser_navigate` | Mutating | Перейти по адресу в активной вкладке | `url`* (http/https) |
| `browser_page` | ReadOnly | Адрес, заголовок и видимый текст страницы — дешевле кадра и точнее для чтения | `max_chars` |
| `browser_screenshot` | ReadOnly | Кадр видимой области — точнее полноэкранного снимка (без обрамления окна) | — |
| `browser_click` | Destructive | Клик по CSS-селектору — не зависит от разрешения | `selector`* |
| `browser_type` | Destructive | Ввод текста через `Input.insertText` — кириллица и не-ASCII надёжны, в отличие от `xdotool type` | `text`*; `selector` |
| `browser_eval` | Destructive | Выполнить JS в контексте страницы (исполнение произвольного кода) | `expression`*; `wait` |

Ограничения, о которых стоит знать: `browser_eval`, `browser_page` и
`window_list` отдают недоверенный текст — их вывод проходит warn-детектор
инъекций (`docs/threat-model.md`); текст, **отрисованный в картинку**, детектор
не видит в принципе, и это закрыто классом `Destructive` и выключенным по
умолчанию контуром, а не регекспом. Фоновым субагентам инструменты воздействия
не выдаются вовсе (ADR-041).
...
`arch-ml harness-run <имя> --repo <путь>`.
