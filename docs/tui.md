...
| Правая панель | Вкладки `◇ Mermaid` / `✓ Рубрика` / `◈ Знания` / `▶ Флот`: последний рендер диаграммы, отчёт рубрики, результат поиска, dashboard прогона флота. При отсутствии журналов прогонов панель явно сообщает об этом — прежний кадр не сохраняется. Ширина адаптивная, кап 60% экрана; `F5` скрывает панель целиком (узкие терминалы). |
| Строка состояния | Появляется только когда есть что сказать: «⠸ выполняется: bash: cargo test · 0:42 · Esc — прервать» (ход инструмента), «⠸ отвечает · 0:23 · ~312 ток · ~26 т/с» (стрим ответа: объём и скорость), «⠸ модель думает · 1:23» (разгон модели; во время стрима «мыслей» — с объёмом и скоростью), «очередь: N». В простое места не занимает. |
