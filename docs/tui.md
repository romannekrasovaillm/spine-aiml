...
| Зона | Содержимое |
|---|---|
| Диалог (центр, мин. 24 кол.) | Блоки: реплики user/assistant, «мысли» модели, вызовы инструментов, результаты слэш-команд, ошибки. **Проза верстается в меру чтения** — не шире `MAX_READING_WIDTH` = 100 колонок (`src/tui/render.rs::reading_width`), независимо от ширины терминала: строка в 150 колонок не удерживается глазом, а текстовая колонка перестаёт переверстываться при открытии правой панели. Арт (mermaid, ASCII-схемы) мерой не ограничен — он не переносится, а клипается. Тело ответа идёт под желобком `▎ `, и строка-продолжение получает тот же желобок — перенос абзаца читается как продолжение, а не как новый абзац (перенос с висячим отступом делает слой вёрстки, `src/tui/text.rs::markdown_lines`, где ещё известен маркер строки: буллет, цитата, код). Лимит — 500 блоков (`MAX_BLOCKS`), старые отбрасываются. Вызовы инструментов — компактный «журнал активности»: серия подряд идущих вызовов схлопывается (первые 2 + счётчик + последние 3), каждый вызов — одна строка с именем и кратким действием из аргументов (путь/команда/запрос), итог — одна строка у последнего завершённого и у ошибок; «мысли» модели — одна строка + счётчик скрытого в шапке блока; полный вывод — на вкладках правой панели. Когда история длиннее экрана, в заголовке справа — счётчик позиции `строка/всего`, слева — полоса скролла; поиск по диалогу — `Ctrl+F` (§7). |
| Правая панель | Вкладки `◇ Mermaid` / `✓ Рубрика` / `◈ Знания` / `▶ Флот` / `◉ Субагенты`: последний рендер диаграммы, отчёт рубрики, результат поиска, dashboard прогона флота, список фоновых субагентов текущей сессии. Вкладка «Субагенты» обновляется сама, пока открыта, и берёт данные из реестра инструмента `subagent_run`; пустая вкладка прямо подсказывает, чем запускать субагентов. При отсутствии журналов прогонов панель явно сообщает об этом — прежний кадр не сохраняется. Ширина адаптивная, кап 60% экрана; `F5` скрывает панель целиком (узкие терминалы). |
| Строка состояния | Появляется только когда есть что сказать: «⠸ выполняется: bash: cargo test · 0:42 · Esc — прервать» (ход инструмента), «⠸ отвечает · 0:23 · ~312 ток · ~26 т/с» (стрим ответа: объём и скорость), «⠸ модель думает · 1:23» (разгон модели; во время стрима «мыслей» — с объёмом и скоростью), «очередь: N», «гипотезы: …» (строка теплицы после постановки цели, см. ниже). В простое места не занимает. |
...
Рядом — таймер хода (`модель думает · 1:23`): разгон модели виден, а не
выглядит «офлайном». Когда пошёл стрим, строка конкретнее: `отвечает ·
0:23 · ~312 ток · ~26 т/с` (объём — грубая оценка 4 байта ≈ 1 токен, как
`ChatMessage::rough_tokens`; во время стрима «мыслей» — тот же формат с
`модель думает`). Когда выполняется инструмент, строка показывает его:
`выполняется: bash: cargo test · 0:42`.

Долгая команда не прячется за немым спиннером: у выполняющегося вызова в
«журнале активности» — живой таймер и хвост вывода (последняя строка, кап
80 знаков, подвеска `↳` под строкой вызова; снапшоты раз в 0,7 с; канал
`ToolContext::progress` →
`AppMessage::ToolProgress`, см. `src/tools/bash.rs`, `src/harness.rs`,
`src/tui/app.rs`). Живой хвост подключён у `bash` (сборка, тесты) и у
синхронного `harness_run` (прогон кодового харнесса: десятки минут —
видно, что именно он сейчас делает; у фонового `background=true` своя
видимость — dashboard флота и per-agent лог).

**Строка теплицы гипотез.** После постановки цели в строке состояния над
вводом появляется первая строка ответа доменного хука `intent` — карточки
гипотез, пересекающиеся с намерением и фактами репозитория
(`docs/hypotheses.md`; `/hypotheses [текст]` печатает её же в диалоге).
Это факт, а не действие: строка называет карточки, которые «поднять»,
и ничего не обещает сделать. Приоритеты: карточка очереди вытесняет
подсказку; во время хода строку занимает живой статус хода, после хода
подсказка возвращается (всё это время та же строка видна в подтверждении
цели в диалоге); при открытой блокирующей ask-модалке подсказки нет —
её «… — поднять?» читалось бы как живой вопрос, которого на экране нет.

Хук вызывается **ровно один раз** — в момент постановки/замены цели, в
исполнителе слэш-команды (`src/agent/slash.rs::intent_line`, таймаут 5 с),
а не на кадр и не на нажатие клавиши; TUI только хранит готовый результат
в поле `App::intent_hint` и рисует его (`src/tui/render.rs::draw_input_state`).
Строка снимается подкомандой `/goal` без нового намерения
(`status`/`pause`/`cancel`/`next`) и командой `/new`. Длинная строка
режется по клеткам с многоточием темы (`fit_label_cells`), без переноса на
второй ряд. В headless-режиме (`arch-ml run --goal`) та же строка уходит
приглушённо в stderr (`» …`) — stdout остаётся чистым для пайпа.
...
- `· кэш N%` — доля промпта последнего ответа, попавшая в prompt-кэш
  провайдера (usage последнего ответа, `AgentEvent::Usage`): **зелёный**
  ≥ 70 %, **оранжевый** 30–69 %, **красный** < 30 % — ранний сигнал взлёта
  стоимости при сломе кэша. Сегмент появляется, только когда провайдер
  прислал `cached_tokens` (`prompt_cache_hit_tokens` у DeepSeek,
...
- **Стриминг**: ответ печатается дельтами по мере генерации; «мысли»
  ...
  видимого ответа. В диалоге от блока остаётся одна строка (последняя
  реплика) и счётчик скрытого в шапке: рассуждение — черновик модели, и
  в полном виде оно конкурирует за внимание с самим ответом.
...
забрать отчёт pull-путём — инструмент `subagent_result`. Текущий список
задач живёт на вкладке «Субагенты» правой панели (`F7`; обновляется сама,
пока открыта). Устройство плагинов и субагентов — `docs/plugins_and_skills.md`.
...
| `F1` / `F2` / `F3` / `F6` / `F7` | вкладки Mermaid / Рубрика / Знания / Флот / Субагенты (прыжок показывает панель, даже если скрыта `F5`) |
...
| Клавиша | Действие |
|---|---|
| `↑/↓` или `j/k`, `Home`/`End`, `PgUp`/`PgDn` | навигация (`PgUp`/`PgDn` — на 10 пунктов) |
| `Enter` | подтвердить выбор |
| `1`–`9` | быстрый выбор по номеру — только у первых девяти пунктов; панель сама считает диапазон по их числу (`1`–`n`), у одного пункта подсказка `1`, у десяти и больше — `1-9 + ↑/↓`; цифра вне списка отвечает тостом, а не молчит |
