...
модели доменной зоны банковской линии Spine (см. `NOTICE.md`), дорожная
карта — `ROADMAP.md` в корне репозитория.

Зона `aiml/` задумана как толстый домен за типизированной границей — по
модели доменной зоны банковской линии Spine (`NOTICE.md`). Ядро (`src/`) не
должно зависеть от `aiml/` (инвариант AD-1 / AD-BE1, перенесённый на новый
домен).
...
дополнительные пресеты и демо (графтинг, дистилляция).

## Целевая структура (по модели доменной зоны банковской линии)
...
4. Эталоном толстого домена остаётся доменная зона банковской линии Spine
   (внешний ориентир, `NOTICE.md`), пока `aiml/` не наберёт критическую
   массу.
