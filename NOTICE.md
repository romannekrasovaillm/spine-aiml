...
- **Лицензии:** ядро — MIT (`LICENSE`). Проприетарная зона `banking/`
  (`LICENSE.banking`, Spine Banking Edition) в эту публикацию **не входит** —
  она остаётся в приватном репозитории владельца; роль эталона толстого
  домена здесь выполняет открытая зона `aiml/` (лицензия MIT, как ядро).
