...
1. **Agentic-базу в штатном режиме** (`evidence/s3am-agentic-baseline.json`) — на том
   ...
   сверенным с замороженной цитатой ADR-033 (константа `BASE_FROZEN`, которую держит
   страж `tools/check_sft_agentic_share.py`) — цитата и артефакт обязаны сойтись,
   иначе сравнивать не с чем. Сам страж с 25.09.2026 цитату не сверяет (ADR-059):
   он проверяет сопоставимость базы и отчёта, а сверку с историческим числом
   делает этот свод. Различие (если есть) объясняется **измерением**, а не
...
#: здесь: одно число в одном месте. Сам страж этим допуском с 25.09.2026 только
#: помечает примечание «цитата — legacy-режим» (ADR-059 п.4); сверку с цитатой,
#: отказ на расхождении и вердикт по базе держит этот свод.
