...
#
# Три исхода:
#   0 — OK (нагрузок ≤ 1) ИЛИ стенд недоступен («gb10 not reachable — check skipped»)
...
# потому что алиасы неравноценны: факт 14.09.2026 — `gb10` (192.0.2.50) из
# рабочей сети недоступен, рабочий `gb10-fast` (192.0.2.2, прямой 10GbE).
# Берётся первый ответивший; если не ответил никто — проверка пропускается с
# exit 0, потому что «стенд недоступен» не равно «инвариант нарушен».
#
...
#
# Запуск: bash tools/check_gb10_serialization.sh [--host HOST] [--hosts "H1 H2"] [--timeout S]
