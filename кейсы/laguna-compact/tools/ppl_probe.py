...
SETS: dict[str, str] = {
    ...
    #: Расширенный общий набор (ADR-025 п.4, дельта S3q): 200 документов, 154 тыс.
    #: токенов, чист против всех кандидатных миксов. Дописан сюда, а не заведён
    #: вторым прибором, потому что методика у него та же — и именно поэтому его
    #: число сопоставимо с v1 внутри одного прогона (реестр: docs/specs/EVAL-SETS.md).
    "v3_general": "datasets/general_eval_v3.txt",
    #: Компонента K2 меры общего языка (ADR-027, дельта S3t): 200 документов русской
    #: прозы **вне** обучающего распределения (аудиторско-бюджетная проза, корпус
    #: Счётной палаты РФ). Тоже дописан сюда, а не заведён вторым прибором: мера
    #: двухкомпонентна, но прибор один — иначе числа компонент несопоставимы по
    #: построению. Реестр ролей: docs/specs/EVAL-SETS.md.
    "k2_general": "datasets/general_eval_k2.txt",
    #: Домен-набор из **другого корпуса** (S3ao, ADR-043 п.3): 200 документов
    #: русских ML/AI-разборов и отчётов из архива `~/Документы/КОД/gigachat/РАЗБОРЫ`.
    #: Дописан сюда, а не заведён вторым прибором: шкала домена одна, и сравнивать
    #: v3 с v2 можно только внутри одного прогона. Реестр ролей — docs/specs/.
    "v3_domain": "datasets/domain_eval_v3.txt",
}
