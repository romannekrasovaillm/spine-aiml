...
/// управляющие символы из текста перед показом в TUI.
///
/// Сырой вывод дочерних процессов (харнесс-прогоны hr-*, субагенты) несёт
/// ESC-байты цвета; записанные ratatui в кадр как есть, они исполняются
/// терминалом — отсюда «мусор» цветных блоков и разъехавшаяся разметка
/// (инцидент 2026-09-23: отчёт фоновой задачи с ANSI в диалоге).
/// Сохраняются `\n` и `\t`; чистый текст возвращается без копирования.
pub(crate) fn sanitized(text: &str) -> Cow<'_, str> {
    if text
        .chars()
...
pub(crate) fn markdown_lines(text: &str, theme: &Theme, width: usize) -> Vec<Line<'static>> {
    ...
    // ANSI ESC-байты — снимаем до разбора, иначе терминал исполнит их
    // при записи кадра (см. `sanitized`).
...
mod tests {
    ...
    #[test]
    fn sanitized_passes_clean_text_without_copy() {
        let clean = "обычный текст\nс переносом\tи табом";
        assert!(matches!(sanitized(clean), Cow::Borrowed(_)));
        assert_eq!(sanitized(clean), clean);
    }

    #[test]
    fn sanitized_strips_csi_colors() {
        // Формат из лога theseus: цветные маркеры вокруг текста.
        let dirty = "\u{1b}[32m❯ ответ\u{1b}[0m и \u{1b}[90m(мышление)\u{1b}[0m";
        assert_eq!(sanitized(dirty), "❯ ответ и (мышление)");
    }

    #[test]
    fn sanitized_strips_osc_with_bel_and_st() {
        // Гиперссылка OSC 8 с BEL-терминатором и заголовок с ST (ESC \).
        let dirty = "\u{1b}]8;;https://example.com\u{7}ссылка\u{1b}]8;;\u{7}";
        assert_eq!(sanitized(dirty), "ссылка");
        let dirty_st = "\u{1b}]0;заголовок\u{1b}\\текст";
        assert_eq!(sanitized(dirty_st), "текст");
    }

    #[test]
    fn sanitized_strips_charset_and_single_finals() {
        // ESC ( B — выбор кодировки; ESC 7 / ESC M — один финальный байт.
        assert_eq!(sanitized("\u{1b}(Babc"), "abc");
        assert_eq!(sanitized("a\u{1b}7b\u{1b}Mc"), "abc");
    }

    #[test]
    fn sanitized_drops_controls_but_keeps_layout() {
        // BEL, backspace, CR, C1 (NEL) — глотаются; \n и \t — выживают.
        let dirty = "a\u{7}\u{8}\u{85}\nb\tc";
        assert_eq!(sanitized(dirty), "a\nb\tc");
    }

    #[test]
    fn sanitized_eats_unterminated_sequence_at_eof() {
        assert_eq!(sanitized("хвост\u{1b}[31"), "хвост");
        assert_eq!(sanitized("хвост\u{1b}"), "хвост");
    }

    #[test]
    fn sanitized_keeps_char_after_lone_esc() {
        ...
        assert_eq!(sanitized("x\u{1b}я"), "xя");
    }

    #[test]
    fn sanitized_real_world_harness_log_line() {
    ...
    #[test]
    fn markdown_lines_strip_ansi_from_model_text() {
        let theme = Theme::default();
        let lines = markdown_lines("чисто\n\u{1b}[31mкрасным\u{1b}[0m", &theme, 80);
        let all: String = lines.iter().map(line_text).collect();
        assert!(!all.contains('\u{1b}'), "{all:?}");
        assert!(all.contains("красным"), "{all}");
    }
}
