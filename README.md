...
| [axiom](кейсы/axiom/) | — (претрейн) | Состязание длинноконтекстных архитектур: единый контракт прогона, механический вердикт, приватный корпус не покидает контур |
...
- **TUI**: Tokyo Night, mermaid-арт на боковой вкладке, выделение мышью с
  ...
  Ответ верстается в **меру чтения** — не шире 100 колонок
  (`MAX_READING_WIDTH`), тело блока идёт под желобком `▎`, а перенос пункта
  сохраняет висячий отступ: продолжение абзаца не читается как новый абзац.
  «Мысли» модели в диалоге — одна строка со счётчиком скрытого.
