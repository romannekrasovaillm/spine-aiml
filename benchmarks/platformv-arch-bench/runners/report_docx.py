...
def interpret(effects):
    ...
    out.append(f"H1 подтверждена для Spine с ризонингом: spine-arch-think "
               ...
               f"Code и Kimi Code тоже работали с ризонингом (модель "
               f"ризонит по умолчанию, D21) — паритет здесь это честное "
               f"«ризонинг против ризонинга». На GLM-5.3 Flash — "
    ...
    out.append(f"H3 подтверждена: агентный контур важнее выбора харнесса, "
               ...
               f"голой модели; эффект суммирует контур и ризонинг — raw "
               f"на dsf неризонящий алиас deepseek-chat, D21), на "
               f"GLM-5.3 Flash голая модель почти не "
...
def build_docx(summary, records, charts, effects, interim):
    ...
    doc.add_paragraph(
        "Ризонинг по рукам (перепроверено 2026-09-14 по сессионным "
        "журналам и живым API-пробам, D21). На dsf (DeepSeek V4.1 Flash) "
        "модель ризонит по умолчанию — reasoning_content в ответе без "
        "всяких параметров, отключается только явным "
        "thinking:{type:disabled}. Поэтому Claude Code и Kimi Code "
        "работали с ризонингом (у claude — thinking-блоки в сессиях до "
        "~200K символов, у kimi — thinkingEffort high/max в wire-"
        "журналах), как и qwen, omp, theseus/openclaw (max, D11). Без "
        "ризонинга — только spine-arch/spine-min (явный --think off, "
        "D10) и raw-llm (неризонящий алиас deepseek-chat); эталонный "
        "spine-arch-think — явный --think on (64K). На GLM-5.3 Flash "
        "ризонинг включён на стороне модели у всех рук (API Z.AI не "
        "позволяет его отключить — HTTP 1210, только effort low/high/"
        "max; харнессы effort не задавали, у raw-llm — явно effort=low). "
        "На DeepSeek V4 Pro (ризонящая модель по умолчанию) Claude Code, "
